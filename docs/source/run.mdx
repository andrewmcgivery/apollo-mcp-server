...
<Tabs>
    ...
    curl -sSL https://mcp.apollo.dev/download/nix/v1.5.0-experimental.1 | sh
    ```
    ...
    ```terminal showLineNumbers=false
    # Note the `v` prefixing the version number
    iwr 'https://mcp.apollo.dev/download/win/v1.5.0-experimental.1' | iex
    ```
    </Tab>
...
```bash
# Note the `v` prefixing the version number
docker image pull ghcr.io/apollographql/apollo-mcp-server:v1.5.0-experimental.1
```
